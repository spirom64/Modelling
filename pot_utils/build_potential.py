...
def get_args():
    ...
    parser.add_argument('-p', '--pdb', type = ag.FileType('w+'), help = 'Output pdb file name', required = False, default = 'cone.pdb')
...
def sgn(x): 
	""" signum function (more exactly, Heaviside function) """
	if x > 0:
		res = 1
	else:
		if x < 0:
...
def enorm(x): 
	""" returns Euclid's norm of n-dimensional vector """
	norm = 0
	for i in x:
		norm += i**2
	norm = norm**0.5
...
def R(x): 
	""" returns radius of potential surface """
	if x < x1: # in conical area
		rd = (x0 - x)*tg_alpha + r0
	else:
		if x < x0: # in connection area
			rd = a0*(-(x0 - x)**3 + (x0 - x)**2) + r0
		else: # in cylinder area
			rd = r0
	return rd

def make_force(x, y, z): 
	""" Helper. Builds force field depending on coordinates """
...
def make_pot(x): 
	""" Main logic. Builds potential in y0z plane at constant x """
	pdblines = []
	potlines = []
